...
### 7. 环境声学（遮挡与混响）
* **遮挡滤波器：** 如果说话者与听众处于不同的区域或隔间，客户端将自动应用低通滤波器（截止频率 600Hz，音量 65%）以模拟物理遮挡/声学遮挡。截止频率会平滑过渡以防止产生咔哒声。
...
* **命名管道连接：** 客户端通过本地 Windows 命名管道 (`\\.\pipe\discord-ipc-0`) 与 Discord 直接通信，无需引入庞大的外部 NuGet 依赖项。
