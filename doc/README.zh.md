...
* **REST API：** 公开端点“GET /api/status”和“POST /api/action”以进行外部集成。
...
### 1. 启用配套应用程序
默认情况下，伴侣应用本地 HTTP 服务器处于禁用状态，以节省系统资源。要启用它：
1. 打开 XuruVOIP 客户端并单击 **设置** 图标。
2. 在 **常规** 选项卡中，选中 **启用配套 HTTP 服务器** 框。
3. 在**配套服务器端口**下，您可以自定义端口号（默认：`8891`）。
4. 单击“**保存并关闭**”进行应用。 HTTP 服务器现在将在本地启动。您可以在 PC 或移动设备上的任何浏览器中打开“http://localhost:8891”来访问 Web 控制器仪表板。
