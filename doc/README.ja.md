...
### 2. 位置追跡と向きの推定
* **スクリーンキャプチャとOCR：** セッション座標が表示される画面領域を定期的にキャプチャします。画像の前処理後、**Tesseract OCR** によって文字認識されます。
...
* **再生：** デコードされたOpusフレームは、必要に応じて**無線DSPフィルタ**（ヘルメット装着時や無線通信時）を通され、位置調整と音量調整後、NAudioの `MixingSampleProvider` でミキシング再生されます。
...
3. **OCR (文字認識)**：対象モニターの選択、キャプチャ間隔（ms）、スキャン領域の指定、文字抽出プレビュー。
4. **Audio (音声)**：入出力デバイスの選択、音量ゲイン、発話モード（PTT / VAD）、VAD感度設定、**3D空間オーディオ**の有効化（サーバーが許可している場合のみ変更可能）。
