...
|部分|描述 |
| :--- | :--- |
| [📖 详细功能指南](../doc/functionnalities.md) |关于所有 20+ 已实现功能的非技术和技术说明。 |
...
| [💡核心功能概述](#-核心功能概述) |超过 19 个已实现的空间和网络功能的详细分类。 |
...
* 📖 **[详细功能指南](../doc/functionnalities.md)**：深入介绍每个已实现的功能、工作原理、使用方法以及为什么需要它们。
...
* **自定义调制器滑块：** 可通过设置滑块精确调节音调移位、环形调制频率/混合、镶边深度/速率/反馈以及位压缩设置。

### 5. 📻 沉浸式无线电衰减和提示音
* **带通滤波：** 在使用无线电频道或防护面罩放下时，对无线电滤波器进行低/高截止建模。
* **无线电信号衰减：** 当玩家之间的距离接近无线电发射器限制时，截止带变窄并混合带通滤波的静态噪声。
* **原声无线电铃声：** 在无线电频道上进行传输时播放机械式按键开启（key-down）与按键释放（key-up）提示音。支持在设置或伴侣应用程序中选择四种不同的数学合成音效配置文件：军用（正弦扫频）、工业（机械撞击声）、外星（环形调制扫频）和复古（模拟继电器咔嗒声）。
* **基于行星距离的无线电延迟：** 模拟基于光速（约 3.3ms/km，最大 3000ms）的信号传播延迟，以实现逼真的无线电通信延迟。
* **自定义 PTT 提示音：** 加载并重采样 `Resources/` 目录下的自定义 WAV/MP3 文件（`radio_key_down` 和 `radio_key_up`）以作为用户提供的提示音。
...
* **交互式 HUD 自定义器：** 支持通过设置或伴侣应用程序实时自定义 HUD 主题（Aegis、Anvil、Drake、RSI、Origin）、位置（屏幕四角/中央）以及组件可见性（微型雷达、活动发言者列表、连接频道页眉）。
...
* **3D 高度指示器：** 当垂直高度差达到 2 米或更多时，在雷达图标旁附加垂直方向箭头和甲板高度差（例如：`Bob (▲ 12m)`）。
* **语音转文本字幕：** 使用离线轻量级 Whisper 模型 (`ggml-tiny.bin`) 将传入的无线电/接近音频转录为本地化的 HUD 字幕。
* **免提 PTT 语音命令：** 按住专用的语音命令键可暂时静音发送的接近/无线电语音，并缓存麦克风音频。松开按键后，语音将通过本地 Whisper 模型进行转录，以触发飞船操作：
  * **支持的命令：** 面罩/头盔切换、麦克风静音/取消静音（接近/无线电/配置文件/全部）、活动无线电频道选择以及变声器预设切换。
  * **多语言关键字匹配：** 支持 8 种语言（英语、法语、德语、西班牙语、葡萄牙语、日语和中文）。
  * **置信度阈值过滤：** 可配置的滑块可以过滤掉低置信度匹配或无关语音。
  * *默认禁用；启用时如果在本地未检测到 Whisper 离线转录模型（约 140MB），则会自动在后台进行下载。*
  * **语音命令参考列表：**
    以下是所有支持的免提 PTT 语音命令列表，分为 XuruVOIP 应用程序控制和星际公民游戏按键绑定：
    
    #### XuruVOIP 应用程序控制（XuruVOIP 专属）
    这些命令直接在 XuruVOIP 内控制您的头盔/面罩状态、通道静音、无线电频率选择或声音调制：
    * **切换面罩/头盔滤波器：** `头盔`, `面罩`, `切换头盔`, `切换面罩`
    * **静音近距通道：** `静音近距`, `近距离静音`
    * **取消静音近距通道：** `取消静音近距`, `近距离取消静音`
    * **静音无线电通道：** `静音无线电`, `无线电静音`
    * **取消静音无线电通道：** `取消静音无线电`, `无线电取消静音`
    * **静音配置文件通道：** `静音配置文件`, `配置文件静音`
    * **取消静音配置文件通道：** `取消静音配置文件`, `配置文件取消静音`
    * **静音麦克风（全局）：** `全部静音`, `静音麦克风`, `麦克风静音`
    * **取消静音麦克风（全局）：** `取消全部静音`, `启用麦克风`, `麦克风取消静音`
    * **切换无线电频道：** `切换频道`, `设置频道`, `频道` 后跟频道名称（例如：*"切换频道 Alpha"*）
    * **设置变声器配置文件：** `变声器`, `声音配置文件`, `变声` 后跟配置文件名称：`alien` (或 `外星人`), `cyborg` (或 `半机械人`, `改造人`), `robotic` (或 `机器人`), `pitchshift` (或 `音高`), `none` (或 `无`, `关闭`)
    
    #### 模拟飞船控制（星际公民相关）
    这些命令在您的电脑上模拟物理按键以触发星际公民的飞船操作：
    * **切换飞船电源：** `电源`, `切换电源`, `系统开启`, `系统关闭`
    * **切换飞船引擎：** `引擎`, `切换引擎`, `开启引擎`, `关闭引擎`
    * **切换武器系统：** `武器`, `切换武器`, `开启武器`, `关闭武器`
    * **切换护盾 (ON/OFF)：** `切换护盾`
    * **均衡/重置护盾：** `重置护盾`, `均衡护盾`
    * **切换VTOL模式：** `vtol`, `切换vtol`
    * **量子引擎启动 (Quantum)：** `量子`, `量子引擎`, `量子航行`
    * **定速巡航开关 (Cruise Control)：** `巡航`, `定速巡航`, `巡航控制`, `切换巡航`
    * **请求降落 / 开启机库：** `请求降落`, `申请降落`, `开启机库`, `呼叫atc`
    * **切换飞船舱门/外门：** `舱门`, `外部`, `开启舱门`, `关闭舱门`, `开启外部`, `关闭外部`
    * **护盾向前偏转：** `护盾`, `护盾向前`, `前部护盾`, `偏转护盾`
    * **切换起落架：** `起落架`, `收放起落架`, `放下起落架`, `收起起落架`
    * **飞行模式 (SCM/NAV)：** `飞行模式`, `航行模式`, `导航模式`
    * **扫描模式：** `扫描模式`, `开启扫描`, `扫描仪`
    * **采矿模式：** `采矿模式`, `开启采矿`
    * **打捞模式：** `回收模式`, `打捞模式`
    * **武器分配最大化：** `武器分配`, `武器最大`, `最大武器`
    * **护盾分配最大化：** `护盾分配`, `护盾最大`, `最大护盾`
    * **引擎分配最大化：** `引擎分配`, `引擎最大`, `最大引擎`
    * **重置/平衡分配：** `重置分配`, `平衡分配`
    * **发射干扰弹：** `干扰弹`, `发射干扰弹`, `诱饵`
    * **发射噪声弹：** `噪声弹`, `发射噪声弹`, `铝箔`
    * **切换飞船大灯：** `大灯`, `切换大灯`, `开启大灯`, `关闭大灯`
    * **锁定最近敌机：** `锁定敌机`, `锁定最近敌机`, `最近敌机`
    * **循环子系统：** `循环子系统`, `瞄准子系统`, `瞄准引擎`, `瞄准护盾`
    * **切换辅助瞄准：** `辅助瞄准`, `切换辅助瞄准`, `云台模式`
    * **钉住目标：** `钉住目标`, `保存目标`
    * **切换脱钩模式：** `无辅助驾驶`, `脱钩模式`, `解耦模式`
    * **切换G力安全：** `g力安全`, `切换g力安全`, `防晕厥安全`
    * **切换限速器：** `速度限制`, `切换速度限制`, `限速器`
    * **增加干扰弹数量：** `增加干扰弹数量`, `更多干扰弹`
    * **重置干扰弹数量：** `重置干扰弹数量`, `重置干扰弹`
    * **擦拭面罩：** `擦拭面罩`, `清洁面罩`, `擦面罩`
    * **呼叫目标：** `呼叫目标`, `呼叫飞船`
...
* **动态频率跟踪：** 自动移动 Discord 语音桥连接，以跟踪并同步已配置领导者或 Command/Leader 配置文件玩家的活动无线电频道。
...
### 15. 🔌 外部硬件遥测 (Sim-Pit UDP 同步)
* **实时 UDP 同步：** 启用后，客户端每 100 毫秒以 JSON 格式向 `127.0.0.1:8895`（可配置）广播其 VoIP 和头盔状态。
* **遥测有效载荷：** 包括本地发送状态（`IsTransmittingProximity`、`IsTransmittingRadio`）、远程接收状态（`IsReceivingProximity`、`IsReceivingRadio`）、护目镜状态（`HelmetVisorDown`）、活动频道和当前区域。
* **硬件集成准备：** 允许模拟座舱（sim-pit）构建者连接自定义的 Arduino LED、Stream Deck 或物理警示灯，以实时响应活动通信。

### 16. 🪐 行星大气密度模拟
* **音量范围缩放：** 近距离语音通信范围根据行星大气密度动态调整（例如：在 Cellin 上的衰减速度加快 3.5 倍，而在 Crusader 上的衰减速度减慢 0.75 倍）。
* **稀薄气体闷音：** 在无空气或薄大气的卫星户外，近距离语音会通过数字低通滤波器进行闷音处理（例如 Cellin 上的截止频率为 800Hz）。
* **舱内增压旁路：** 在飞船客舱、设施或太空站内部，此模拟会自动旁路。

### 17. 🎙️ 任务后语音记录器和 AAR 门户
* **零开销 Ogg/Opus 容器：** 直接将原始 VoIP Opus 数据包保存到磁盘中的 `.ogg` 音频文件中，可在浏览器中直接播放，占用的磁盘空间比 MP3 少 5 倍，且对服务器 CPU 零编码压力。
* **针对管理员的录制：** 仅对通过管理员仪表板启用的活动近距离、无线电频道或配置文件目标开始录制。
* **画布活动时间线：** 在管理员门户网站的 2D HTML5 画布时间线上可视化语音传输，允许管理员点击对应的时间块进行播放或删除片段。

### 18. 📞 舰船间呼叫与呼叫系统
* **驾驶舱对驾驶舱呼叫：** 在 5,000 米的距离限制内，建立舰船之间的直接私密通信环路。
* **免提呼叫流：** 在呼叫期间通过语音活动检测 (VAD) 自动捕获语音，绕过标准近距离/无线电一键通 (PTT) 热键。
* **逼真拨号音效：** 通过 NAudio 合成逼真的拨号扫频音、呼叫等待音、连接和断开提示音。

### 19. 🔤 护目镜 HUD 实时翻译字幕
* **动态短语翻译：** 使用针对 7 种支持语言的本地化飞行/战斗字典，实时翻译传入的外语语音流。
* **HUD 字幕前缀：** 直接在护目镜 HUD 上显示翻译后的文本，并带有 `[源语言 -> 目标语言]`（例如 `[FR -> EN]`）的前缀。
* **按需 Whisper 加载：** 检查本地是否存在 Whisper 模型，如果不存在则显示警告并自动在后台异步下载。

### 20. 🎧 双耳 HRTF 空间音频
* **物理耳道模拟：** 利用 ITD（双耳时间差）和 ILD（双耳声级差）低通衰减，模拟人耳形状和头影效应。
* **立体声兼容性：** 无需环绕声硬件支持，即可在标准立体声耳机上提供高保真度的 3D 空间音频定位。

### 21. 📊 护目镜 HUD 3D 频谱图
* **FFT 遥测叠加：** 实时对传入的话者语音流计算 Radix-2 64 点快速傅里叶变换 (FFT)。
* **动态 HUD 可视化：** 在 Vulkan/DX HUD 上的活动发言者名字旁，将音频频率分组为 8 个光谱带显示，并带有平滑衰减效果。

### 22. 🎙️ 语音控制飞船操作
* **语音命令转热键：** 听取语音指令（例如 "open doors"），并与 8 种语言的本地化字典进行匹配。
* **直接硬件键盘输入：** 使用低级 Win32 `keybd_event` API 模拟物理按键（按键保持 50 毫秒以确保游戏可靠捕获，支持组合键）。

### 23. 🛰️ 服务器端 AAR 3D 回放
* **坐标记录：** 服务器每 500 毫秒将玩家坐标和区域记录到 `<session_id>_positions.jsonl` 文件中。
* **同步 WebGL 3D 回放：** 在交互式 Three.js WebGL 3D 地图上可视化玩家的 3D 轨迹和发言脉冲环（支持鼠标旋转、平移和缩放），与录制的 Ogg/Opus 音频完全同步。
...
3. **位置**：切换位置源（“OCR 屏幕扫描�您可以将以下 19 个操作中的任意一个拖放到 Stream Deck 键上：
* 🎤 **接近静音**：切换传出接近麦克风静音。
* 📻 **无线电静音**：切换传出无线电麦克风静音。
...
* 🔄 **循环广播**：循环播放可用的广播频道。
* 📢 **PA Broadcast**：用于在飞船公共广播 (PA) 系统上广播的单发式 (Push-to-Talk) 按键。
* 📡 **Beacon Mode**：切换无线电中继/信标模式。
* 🎙️ **Voice Command Macro**：触发在后台模拟执行的自定义语音命令宏（可在属性中配置）。
* 💬 **Intercom Status**：显示飞船内部通话状态（`NORMAL`, `SHIELD HIT`, `CRIT PWR`, `QUANTUM`），按下按键可在模拟状态之间循环。
* 🗺️ **Location Telemetry**：在按键上实时显示当前系统区域和三维坐标数据 $(X, Y, Z)$。
* 📞 **Initiate Hail**：向最近的玩家发起舰船间呼叫。
* 📞 **Accept/Answer Hail**：接受传入的舰船间呼叫。
* 📞 **Decline/End Hail**：拒绝传入的呼叫或挂断当前的活动呼叫。
* 🔤 **Toggle Translation**：切换 HUD 实时翻译字幕的开启/关闭。
* 🎧 **Toggle HRTF**：切换实时 HRTF 双耳空间音频定位的开启/关闭。
* 📊 **Toggle Spectrogram**：切换护目镜 HUD 实时 3D 频谱图的开启/关闭。器（开始）
...
### 3. 添加和配置操作
您可以将以下 17 个操作中的任意一个拖放到 Stream Deck 键上：
* 🎤 **接近静音**：切换传出接近麦克风静音。
* 📻 **无线电静音**：切换传出无线电麦克风静音。
* 👤 **配置文件静音**：切换传出配置文件麦克风静音。
* 🔊 **音频邻近静音**：切换传入邻近播放静音。
...
#### 配置（属性检查器）：
对于拖到键上的每个操作，单击它并在底部的 **Property Inspector** 面板中配置参数：
* **配套端口**：配置为与 WPF 客户端设置中配置的端口一致（默认值：`8891`）。
* **Voice Command**（仅限 Voice Command Macro）：输入要执行的文本命令（例如 `"close visor"`, `"open hangar"`）。
* **动态反馈**：操作会自动实时更新其图标和状态。切换显示青色/红色，Intercom Status 循环 4 个状态，而 Location Telemetry 显示坐标。
* **实时频率显示**：**循环收音机**键将直接在物理按钮上实时动态显示当前活动的频率名称！
