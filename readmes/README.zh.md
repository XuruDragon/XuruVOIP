...
* **环境驾驶舱报警注入：** 当“启用报警注入”激活且飞船进入警告状态（护盾受击或临界电力）时，客户端会自动合成低振幅（<0.01）的警报声音（扫频紧急警报循环或快速双嘀警告循环），并将其直接混入输出的麦克风流中，以提供逼真的驾驶舱紧急情况音频反馈。
...
* **同步回放画布：** 在基于 Web 的 HTML5 Canvas 地图上可视化玩家的移动轨迹和发言脉冲环，与录制的 Ogg/Opus 音频完全同步。
