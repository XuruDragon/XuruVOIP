...
* **カスタムモジュレータースライダー:** パラメーター設定用のスライダーにより、ピッチ、リング変調周波数/ミックス、フランジャーの深さ/周波数/フィードバック、およびビットクラッシュの設定を細かく調整できます。
...
* **惑星間距離に基づく無線遅延:** 光速（約3.3ms/km、最大3000ms）に基づく信号伝搬遅延をシミュレートし、リアルな無線通信ラグを再現します。
* **カスタムPTTチャイム:** `Resources/` フォルダー内のカスタムWAV/MP3ファイル（`radio_key_down` と `radio_key_up`）を読み込んで48kHzモノラルにリサンプリングし、ユーザー提供のチャイムとして使用します。
...
* **3D高度インジケーター:** 高度差が2メートルを超える場合、レーダー上に上下矢印と高さの差（例: `Bob (▲ 12m)`) を表示します。
...
* **ハンズフリー PTT 音声コマンド:** 専用の音声コマンドキーを押し続けている間、送信される近接/ラジオ音声が一時的にミュートされ、マイク音声がバッファリングされます。キーを離すと、音声が Whisper モデルによってローカルでテキスト化され、船の操作アクションが実行されます。
  ...
  * **音声コマンドのリファレンスリスト:**
    以下は、サポートされているハンズフリーPTT音声コマンドのリストです。XuruVOIPアプリケーション制御とStar Citizenゲームキーバインドに分かれています。
    
    #### XuruVOIPアプリケーション制御 (XuruVOIP専用)
    これらのコマンドは、XuruVOIP内のヘルメット/バイザー状態、チャンネルミュート、無線周波数選択、または音声変調を直接制御します。
    * **バイザー/ヘルメット切り替え:** `バイザー`, `ヘルメット`, `切り替え`, `バイザー切り替え`
    * **近接チャンネルのミュート:** `近接ミュート`, `プロキシミティミュート`
    * **近接チャンネルのミュート解除:** `近接ミュート解除`, `プロキシミティ解除`
    * **無線チャンネルのミュート:** `ラジオミュート`, `無線ミュート`
    * **無線チャンネルのミュート解除:** `ラジオミュート解除`, `無線ミュート解除`
    * **プロファイルチャンネルのミュート:** `プロファイルミュート`, `プロフィールミュート`
    * **プロファイルチャンネルのミュート解除:** `プロファイル解除`, `プロフィール解除`
    * **マイクのミュート (全体):** `全ミュート`, `マイクミュート`, `すべてミュート`
    * **マイクのミュート解除 (全体):** `ミュート解除`, `マイク解除`, `すべてミュート解除`
    * **無線チャンネル切り替え:** `チャンネル変更`, `チャンネルを`, `チャンネル` に続けてチャンネル名 (例: *"チャンネル変更 Alpha"*)
    * **ボイスチェンジャープロファイル設定:** `ボイスチェンジャー`, `音声プロフィール`, `ボイス` に続けてプロファイル名: `alien` (または `エイリアン`), `cyborg` (または `サイボーグ`), `robotic` (または `ロボット`), `pitchshift` (または `ピッチシフト`), `none` (または `オフ`, `なし`, `normal`)
    
    #### 模擬船体制御 (Star Citizen関連)
    これらのコマンドは、PC上の物理的なキー入力をシミュレートしてStar Citizenの操作をトリガーします。
    * **船体電源の切り替え:** `パワー`, `電源切り替え`, `システム起動`, `システム停止`
    * **船体エンジンの切り替え:** `エンジン`, `エンジン切り替え`, `エンジン起動`, `エンジン停止`
    * **兵器の切り替え:** `兵器`, `ウェポン`, `兵器切り替え`, `兵器有効`, `兵器無効`
    * **シールドの切り替え (ON/OFF):** `シールド切り替え`
    * **シールドの均等化:** `シールドリセット`, `シールド均等化`
    * **VTOLモードの切り替え:** `vtol`, `vtol切り替え`
    * **量子ドライブスプール (Quantum):** `量子`, `量子ドライブ`, `クォンタム`
    * **クルーズコントロールの切り替え (Cruise Control):** `クルーズ`, `クルーズコントロール`, `巡航切り替え`
    * **着陸要請 / ハンガー開放:** `着陸要請`, `着陸リクエスト`, `ハンガー開放`, `atc呼出`
    * **船体ドア/ランプの切り替え:** `ドア`, `外部`, `ドア開閉`, `ドアを開ける`, `ドアを閉める`, `外部開放`, `外部閉鎖`
    * **シールドを前方に偏向:** `シールド`, `シールド前方`, `シールド前`, `シールド偏向`
    * **着陸脚の切り替え:** `ランディングギア`, `ギア`, `ランディングギア展開`, `ギア展開`, `ギア格納`
    * **飛行モード (SCM/NAV):** `飛行モード`, `フライトモード`, `ナビゲーションモード`
    * **スキャンモード:** `スキャンモード`, `スキャナー起動`
    * **採掘モード:** `採掘モード`, `マイニングモード`
    * **回収モード:** `回収モード`, `サルベージモード`
    * **武器電力最大化:** `武器電力`, `武器マックス`, `ウェポンマックス`
    * **シールド電力最大化:** `シールド電力`, `シールドマックス`
    * **エンジン電力最大化:** `エンジン電力`, `エンジンマックス`
    * **電力バランス:** `電力リセット`, `電力バランス`
    * **デコイ射出:** `デコイ`, `フレア`, `デコイ射出`, `フレア射出`
    * **ノイズ射出:** `ノイズ`, `チャフ`, `ノイズ射出`, `チャフ射出`
    * **船体ライト切り替え:** `ライト`, `ライト切り替え`, `ライトオン`, `ライトオフ`
...
* **動的周波数追跡:** リーダー（またはCommand/Leaderプロファイルを持つプレイヤー）の無線チャンネル変更に合わせて、Discordボイスブリッジの接続を自動的に移動および同期します。
...
* **同期された WebGL 3D 再生:** 記録された Ogg/Opus オーディオと完全に同期した、インタラクティブな Three.js WebGL 3D マップ（マウスによるドラッグ回転、右クリック平移、スクロールズーム対応）上にプレイヤーの3D軌跡和発言パルス リングを視覚化します。
